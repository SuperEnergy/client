...
public class RingView extends View {
	...
	private void init() {
		...
		if (!isInEditMode()) {
			// 造成错误的代码段
			mRingPaint.setColor(mContext.getResources().getColor(R.color.yellow_70));
		}
		mRingPaint.setStrokeWidth(mHeartPaintWidth);
		mRingPaint.setStyle(Style.STROKE);
		mRingAnimPaint = new Paint(mRingPaint);
		mRingAnimPaint.setColor(mContext.getResources().getColor(R.color.yellow_30));
		...
		if (!isInEditMode()) {
			mHeartBeatPaint.setColor(mContext.getResources().getColor(R.color.yellow_30));
		}
	...
	private int mPreStartAnimAngle = 0;
	...
	private void startRingAnim() {
		mAnimAngle = 0;
		mPreStartAnimAngle = 0;
		new Thread(new Runnable() {

			@Override
			public void run() {
				//开始动画前，高亮几秒钟
				while (mPreStartAnimAngle < 30) {
					mPreStartAnimAngle++;
					mAnimAngle = 359;
					try {
						Thread.sleep(50);
