...
public class MineListActivity extends BaseRVActivity<MinePresenter, BLEDevice> implements MineContract.View {
...
//        devicelist.clear();
        initDatas();
//        if (mUser != null) {
//            mPresenter.getMineList(mUser.getPid());
//        }
    }
...
//        checkEmpty();
    ...
    public void bindSuccess(Boolean result) {
        ...
        BLEAction.getInstance(mBle, MineListActivity.this).setDeviceActiveStatus(true);
        BLEDevice cdevice = App.getInstance().getCurrentDevice();
        String name = cdevice.getBluetoothDevice().getName();
        String mac = cdevice.getBluetoothDevice().getAddress();
...
//        mDataList.remove(App.getInstance().getCurrentDevice());
        adapter.notifyDataSetChanged();
    ...
    private BleScanResultCallback resultCallback = new BleScanResultCallback() {
        ...
        public void onFail() {
            LogUtils.d("开启扫描失败");
            com.ees.chain.domain.Error err = new com.ees.chain.domain.Error();
            err.setCode("define");
        ...
        public void onFindDevice(BluetoothDevice device, int rssi, byte[] scanRecord) {
            ...
            if (devicelist != null) {
                ...
                if (canadd) {
                    ...
                    if (newdevice.getBluetoothDevice()!=null && !StringUtils.isBlank(newdevice.getBluetoothDevice().getName()) && newdevice.getBluetoothDevice().getName().toLowerCase().contains("eescube")) {
        ...
        if (!BluetoothAdapter.getDefaultAdapter().isEnabled()) {
            ...
            if (!openresult) {
                runOnUiThread(new Runnable() {
                                  @Override
                                  public void run() {
                                      Toast.makeText(MineListActivity.this, getString(R.string.err_open_ble), Toast.LENGTH_SHORT).show();
                                  }
                              });
                showError();
                return;
